...
import { mockContracts } from "@/src/mocks/mockContracts";
      ...
      <div className="absolute w-[340px] h-[84px] left-[50%] top-[35%] -translate-x-[50%] -translate-y-[50%] rounded-[24px_0_0_24px] flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-2 w-[312px] h-[28px]">
          <h6 className="font-semibold text-[18px] leading-[28px] text-[#1E1E1E]">
            {contractTitle.split(" ").length > 3
            ? contractTitle.split(" ").slice(0, 3).join(" ") + "..."
            : contractTitle}
          </h6>
        </div>
...
const ContractRow = ({ contract,onEdit,showCheckbox = false }: ContractRowProps) => {
       ...
       {/* Checkbox */}
        ...
        <div className="flex-1 flex items-center skew-x-[12deg] h-full px-4 gap-6">
          ...
          <div className="flex flex-col justify-center w-[140px]">
           ...
           {/* Note ID */}
          <div className="flex flex-col justify-center w-[140px]">
            <span className="text-sm font-semibold text-gray-900">{contract.note.split(" ").slice(0, 2).join(" ")}</span>
          ...
          <div className="flex flex-col justify-center items-center w-[140px]">
            <div
              className={`
                ...
                ${contract.status === "Running" ? "bg-[#D3FCD2] border border-[#22C55E] text-[#22C55E]" : ""}
                ${contract.status === "Reject" ? "bg-[#FFDCDC] border border-[#FF4D4F] text-[#FF4D4F]" : ""}
                ${contract.status !== "Running" && contract.status !== "Reject" ? "bg-blue-100 border border-blue-200 text-blue-600" : ""}
              `}
            >
              {contract.status}
            </div>
...
────────────────────────────────────────────── */
  ...
  const rejectContracts = mockContracts.filter(c => c.status === "Reject").length;
  ...
  return (
    ...
    <div className="px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-8 ">
        ...
        <Card title="Reject" value={rejectContracts} subtitle={`Last created: ${timeAgo(latestContract.createdDate)}`} />
      </div>
