...
const EducationModal: React.FC<EducationModalProps> = ({ education, isOpen, onClose, onSave }) => {
  ...
  useEffect(() => {
    ...
    else setFormData({ name: "", major: "", startDate: "", endDate: "", description: "" });
  }, [education, isOpen]);

  const educationOptions = ["B.Sc in Engineering CSE", "B.Sc in Engineering IT", "B.Sc in Computer Science", "M.Sc in Computer Science", "Diploma"];
  ...
  if (!isOpen) return null;
    ...
    <div className="fixed inset-0 z-60 flex items-center justify-center p-4">
        ...
        <div className="flex flex-col gap-4">
          <ParallelogramInput label="University Name" placeholder="Enter university name" value={formData.name} onChange={(e) => handleInputChange("name", e.target.value)} />
          <ParallelogramInput label="Major" placeholder="Enter Major" value={formData.major} onChange={(e) => handleInputChange("major", e.target.value)} />
        ...
        >
            ...
            >
                ...
                {loading ? <Loader2 className="animate-spin" /> : education ? "Update" : "Add"}
                </span>
            </button>
            </div>
        </div>
